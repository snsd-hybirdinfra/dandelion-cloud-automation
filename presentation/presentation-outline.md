...
| 이진욱 | 서버 / 가상화 | Linux 환경, Docker, Custom WordPress 컨테이너 |
...
- Managed Node에 Docker / WordPress 배포
...
Control Node
   ↓ SSH Key
Managed Nodes
   ├─ Web Node: Docker / WordPress
   └─ Backup Node: Health Check / Backup / Restore
~~~

### 발표 포인트

Control Node를 중심으로 Ansible Playbook을 실행하고, Managed Node에 대해 서버 설정, Docker 설치, wordpress 배포, 상태 점검 및 백업/복구 검증을 수행한다.
...
| 4 | Playbook 기반 Docker / WordPress 배포 | 조민석 |
...
- Docker 설치 결과
- Docker 서비스 상태
- Custom WordPress 및 MariaDB 컨테이너 실행
- curl 접속 결과
