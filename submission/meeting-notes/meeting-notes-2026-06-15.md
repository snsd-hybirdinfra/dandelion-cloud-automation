...
## 10. 추가 결정 사항: B+ Stretch Goal 자동 구축 방향

금일 추가 논의를 통해 B급 확장이 조기에 완료될 경우, 최종 확장 목표를 **OpenStack CLI + Ansible 기반 End-to-End 자동 구축**으로 설정하기로 하였다.

### 10.1 확장 목표

기본 A급과 B급 구현이 완료된 이후, Ansible Playbook에서 OpenStack CLI 명령을 실행하여 노드 생성부터 서비스 배포, 모니터링 확장까지 자동화하는 것을 Stretch Goal로 둔다.

~~~text
Ansible 실행
→ OpenStack CLI 명령으로 네트워크 / 보안그룹 / 인스턴스 생성
→ 생성된 노드 접속 정보 확인
→ Inventory 반영
→ 서버 초기 설정
→ Custom WordPress + MariaDB 배포
→ HAProxy Reverse Proxy 구성
→ Prometheus / Grafana / exporter 구성
...
| 등급 | 구현 범위 | 기준 |
|---|---|---|
| A급 | 준비된 노드 기반 WordPress/MariaDB 배포, 백업/복구 검증 | 2026-06-26까지 완료 목표 |
| B급 | HAProxy Reverse Proxy, HTTPS, Prometheus/Grafana, node_exporter, cAdvisor | 2026-07-10까지 선택 확장 |
| B+ Stretch Goal | OpenStack CLI 명령을 Ansible에 통합하여 노드 생성부터 B급 구성까지 자동화 | A급/B급 완료 후 시도 |

### 10.3 진행 조건

B+ Stretch Goal은 다음 조건을 만족할 경우에만 진행한다.

| 조건 | 내용 |
|---|---|
| A급 완료 | WordPress/MariaDB 배포, Health Check, Backup/Restore 검증 완료 |
| B급 주요 항목 완료 | HAProxy Reverse Proxy 또는 Monitoring 중 주요 항목 구현 완료 |
| 기본 시연 가능 | 발표 가능한 A급/B급 결과 확보 |
| 실패 시 대응 | B+ 자동화가 실패해도 A급/B급 산출물 기준으로 발표 |

### 10.4 OpenStack CLI 자동화 대상
...
| Node Check | openstack server list, openstack server show |

최종적으로는 Horizon 대시보드 수동 구축에 의존하지 않고, OpenStack CLI 기반 인프라 생성과 Ansible 기반 서버 자동화를 하나의 흐름으로 연결하는 것을 목표로 한다.
