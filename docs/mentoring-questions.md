...
### Q4. B급 기능의 우선순위가 적절한가?

현재 B급 선택 확장 순서는 아래와 같이 설정하였다.
...
A급 안정성을 해치지 않으면서 평가에 도움이 되는 순서로 구성하려고 한다.

멘토님 관점에서 위 순서가 적절한지, 또는 Prometheus/Grafana, Cinder, HAProxy 중 우선순위를 조정하는 것이 좋은지 확인받고 싶다.

---

## 2. 인프라 구성 관련 질문

### Q5. Control Node 1대 + Managed Node 2대 구성이 적절한가?
...
### Q6. OpenStack 네트워크 구성을 어느 수준까지 보여주는 것이 좋은가?
...
### Q7. 자원 부족이 발생할 경우 어떤 조치 방향이 적절한가?
...
DB 원본 데이터를 Cinder에 직접 올리는 방식은 Docker Volume, DB 권한, 마운트 안정성 문제로 인해 제외하고자 한다.
...
### Q9. Ansible Playbook 범위는 Docker Compose 기반 WordPress/MariaDB 배포까지가 적절한가?
...
### Q10. Playbook을 단일 site.yml로 구성해도 되는가?
...
### Q11. OpenStack CLI + Ansible End-to-End 자동화는 후순위 추가 도전으로 두어도 되는가?
...
다만 이 구성은 인증 정보, OpenStack CLI 환경, 중복 리소스 처리, inventory 반영 등 변수가 많아 현재는 B+의 핵심이 아니라 후순위 추가 도전으로 분리하려고 한다.
...
### Q12. Web Node 2대 + DB Node 분리 + HAProxy LB를 B+ Stretch Goal로 두는 것이 적절한가?

A급과 B급이 조기에 안정화될 경우, 아래와 같은 확장 구조를 B+ Stretch Goal로 검토하고 있다.
...
이 구조는 단일 Web Node 구조보다 인프라적으로 확장성이 좋아 보이지만, 구현 난이도와 검증 범위가 커질 수 있어 필수 구현이 아닌 B+로 분리하였다.

이 범위 설정이 적절한지 확인받고 싶다.

---

### Q13. DB Node 구현 시 Cinder Volume 처리 방향은 어떻게 잡는 것이 적절한가?

B+ Stretch Goal로 Web Node 2대와 DB Node 분리 구조를 검토하고 있다.

이 경우 MariaDB 데이터를 어디에 저장할지에 대한 범위 설정이 필요하다고 판단하였다.
...
→ WordPress Container
→ DB Node의 MariaDB에 접속

DB Node
→ MariaDB Container
→ DB 원본 데이터는 Docker Volume 또는 로컬 디스크 사용
...
MariaDB 원본 데이터를 Cinder Volume에 직접 올리는 방식도 가능해 보이지만, Docker Volume 경로, DB 권한, mount 안정성, 장애 시 복구 절차가 복잡해질 수 있다고 판단하였다.

따라서 이번 프로젝트에서는 Cinder Volume을 DB 원본 데이터 저장소가 아니라 Backup / Validation Node의 백업 저장소로 활용하고, DB Node의 데이터는 dump 방식으로 백업하는 방향을 고려하고 있다.

이 범위 설정이 기본 프로젝트 수준에서 적절한지 확인받고 싶다.

특히 DB Node 분리 후에도 Cinder Snapshot이나 DB 원본 디스크 분리는 제외하고, MariaDB dump 기반 백업/복구 검증으로 제한해도 되는지 조언을 받고 싶다.

---

### Q14. WordPress files 자동 동기화는 제외하는 것이 적절한가?
...
대신 B+ 시연은 HAProxy roundrobin, Web-1/Web-2 응답 분산, 공통 DB 연결 확인 수준으로 제한하려고 한다.

이 범위 통제가 적절한지 확인받고 싶다.

---

### Q15. OpenStack LBaaS / Octavia를 제외하고 HAProxy 컨테이너 LB로 검증하는 것이 적절한가?
...
따라서 이번 프로젝트에서는 OpenStack LBaaS / Octavia를 제외하고, HAProxy 컨테이너 기반 Reverse Proxy 또는 간단한 Load Balancing으로 검증하려고 한다.
...
### Q16. 운영 중 발생할 수 있는 장애 시나리오는 어떤 것을 포함하는 것이 좋은가?
...
### Q17. 리소스 부족 시 트러블슈팅 절차는 어떻게 잡는 것이 적절한가?
...
### Q18. GitHub Actions 기반 산출물 상태 자동 갱신은 차별성으로 어필해도 되는가?
...
### Q19. 제출 산출물에서 GitHub 문서와 Google Drive 자료를 어떻게 연결하는 것이 좋은가?
...
### Q20. 현재 프로젝트에서 반드시 줄여야 할 범위가 있는가?
...
### Q21. 현재 프로젝트에서 추가하면 평가에 도움이 될 최소 기능이 있는가?
...
6. Web Node 2대 + DB Node 분리 + HAProxy LB
~~~
