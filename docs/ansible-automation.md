...
Phase 1에서는 Docker 설치, MariaDB 배포, Custom WordPress 배포, HAProxy HTTP Reverse Proxy 배포를 자동화한다.
...
|---|---|
| Phase 1 | Docker 설치, DB Node MariaDB 배포, Web Node WordPress 배포, Proxy Node HAProxy HTTP Reverse Proxy 배포 |
...
| DB 컨테이너 | ansible db -a "docker ps" | dandelion-mariadb running |
