...
본 프로젝트는 OpenStack 기반 Ubuntu 인스턴스 위에서 Ansible을 활용하여 Proxy, Web, DB, Backup / Validation 계층을 분리하고, Docker Compose 기반 WordPress/MariaDB 서비스 배포, HAProxy Reverse Proxy, 상태 점검, 백업 및 복구 검증을 수행하는 구조를 가진다.
...
| DB Node | WordPress 데이터 저장 | MariaDB Container |
...
| Database Layer | MariaDB Container를 DB Node에서 분리 실행 |
...
| DB Node | Docker 설치, MariaDB 컨테이너 배포 |
...
|---|---|
| MariaDB dump | DB Node의 WordPress database dump |
...
| DB | DB Node에서 MariaDB 컨테이너 running 확인 |
