...
인스턴스별 vCPU, Disk, RAM 산정 기준은 별도 문서에서 관리한다.
...
~~~text
Backup / Validation Node
→ DB Node mysqldump 기반 mysqldump 기반 MariaDB dump
...
|---|---|
| mysqldump 기반 mysqldump 기반 MariaDB dump | DB Node의 WordPress database dump |
...
| Backup | backup.sh |
| DB Backup | mysqldump 기반 mysqldump 기반 MariaDB dump |
...
→ /backup mount
→ mysqldump 기반 mysqldump 기반 MariaDB dump 저장
