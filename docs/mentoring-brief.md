...
| 등급 | 기준 |
|---|---|
| A급 | 필수 구현 범위. OpenStack, SSH, Ansible, Docker/WordPress, Health Check, Backup/Restore, 산출물 정리 |
...
Prometheus/Grafana 같은 모니터링 도구를 추가하면 전문성은 높아질 수 있지만, OpenStack 인프라 구성, Ansible 실행, Docker/WordPress 배포, 백업/복구 검증이 안정적으로 완료되지 않은 상태에서 범위를 확장하면 프로젝트 완성도가 떨어질 수 있다.
...
| 구분 | 목표 완료일 | 내용 |
|---|---|---|
| A급 필수 구현 | 2026-06-26 | OpenStack 인프라, Ansible 자동화, Docker/WordPress 배포, 상태 점검, 백업/복구 검증 |
| B급 선택 확장 | 2026-07-10 | HTTPS, Cinder/NFS, node_exporter, Prometheus/Grafana 등 가능한 항목 |
