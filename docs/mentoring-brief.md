...
| 핵심 목표 | OpenStack 기반 클라우드 인프라 구성 이후 Ansible을 활용하여 Proxy / Web / DB / Backup 계층을 분리하고, Docker 기반 WordPress/MariaDB 서비스 배포, 상태 점검, 백업 및 복구 검증을 자동화 |
...
HTTPS, Cinder Backup Volume, Prometheus/Grafana 같은 확장 기능을 추가하면 전문성은 높아질 수 있다. 그러나 OpenStack 인프라 구성, Ansible 실행, Proxy / Web / DB / Backup Node 분리, Docker Compose 기반 WordPress/MariaDB 배포, 백업/복구 검증이 안정적으로 완료되지 않은 상태에서 범위를 확장하면 프로젝트 완성도가 떨어질 수 있다.
...
| Q8 | Ansible Playbook 범위는 Docker Compose 기반 WordPress/MariaDB/HAProxy 배포까지가 적절한가? |
